namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficWaypointRoute : MonoBehaviour
    {
        ...
        public bool randomSpawnFromPool = false;
        ...
        {
            if (randomSpawnFromPool)
            {
                //spawnTrafficVehicles = new GameObject[maxDensity];
                for (int i = 0; i < spawnTrafficVehicles.Length; i++)
                {
                    int randomPoolIndex = UnityEngine.Random.Range(0, AITrafficController.Instance._AITrafficPool.trafficPrefabs.Length);
                    spawnTrafficVehicles[i] = AITrafficController.Instance._AITrafficPool.trafficPrefabs[randomPoolIndex];
                }
            }
            if (useSpawnPoints)
            {
        ...
        private void DrawGizmos(bool selected)
        {
            ...
            if (m_AITrafficGizmoSettings.canUpdateGizmos)
            {
                if (waypointDataList.Count > 2)
                {
                    ...
                    Gizmos.color = selected ? m_AITrafficGizmoSettings.selectedYieldTriggerColor : m_AITrafficGizmoSettings.yieldTriggerColor;
                    for (int i = 0; i < waypointDataList.Count; i++) // Draw Arrows to junctions
                    {
