                    ...
                    for (int i = 0; i < _routes.Length; i++)
                    {
                        ...
                        if (_routes[i].waypointDataList.Count > 0)
                        {
                            ...
                            if (handleDrawDistance >= Vector3.Distance(Camera.current.transform.position, pointTransformPosition))
                            {
                                ...
                                {
                                    style.normal.textColor = YT_configureRouteIndex == i ? laneConnectorHandleTextActiveColor : laneConnectorHandleTextColor;
                                    Handles.color = YT_configureRouteIndex == i ? laneConnectorHandleActiveColor : laneConnectorHandleColor;
                                    if (Handles.Button(pointTransformPosition, Quaternion.LookRotation(sceneViewCameraTransform.forward, sceneViewCameraTransform.up), size, size, Handles.DotHandleCap))
                                    {
                                    ...
                                    if (YT_configureRouteIndex == i)
                                    {
                                        for (int j = 0; j < _routes[i].waypointDataList.Count - 1; j++)
                                        {
                                            ...
                                            {
                                                style.normal.textColor = YT_configureWaypointIndex == j ? laneConnectorHandleTextActiveColor : laneConnectorHandleTextColor;
                                                Handles.color = YT_configureWaypointIndex == j ? laneConnectorHandleActiveColor : laneConnectorHandleColor;
                        ...
                        for (int i = 0; i < _routes.Length; i++)
                        {
                            if (_routes[i].waypointDataList.Count > 0)
                            {
                                    ...
                                    )
                                {
                                    ...
                                    if (handleDrawDistance >= Vector3.Distance(Camera.current.transform.position, pointTransformPosition))
                                    {
                                        style.normal.textColor = (YT_routeInfoIndex == i) ? signalConnectorHandleTextActiveColor : signalConnectorHandleTextColor;
                                        Handles.color = (YT_routeInfoIndex == i) ? signalConnectorHandleActiveColor : signalConnectorHandleColor;
                                        if (Handles.Button(pointTransformPosition, Quaternion.LookRotation(sceneViewCameraTransform.forward, sceneViewCameraTransform.up), size, size, Handles.DotHandleCap))
                                        {
